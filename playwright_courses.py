...
with sync_playwright() as playwright:
    ...
    courses_text_check = page.get_by_test_id('courses-list-toolbar-title-text')
    expect(courses_text_check).to_be_visible()
    expect(courses_text_check).to_have_text("Courses")

    no_results_text_check = page.get_by_test_id('courses-list-empty-view-title-text')
    expect(no_results_text_check).to_be_visible()
    expect(no_results_text_check).to_have_text("There is no results")

    icon_text_check = page.get_by_test_id('courses-list-empty-view-icon')
    expect(icon_text_check).to_be_visible()

    results_display_check = page.get_by_test_id('courses-list-empty-view-description-text')
